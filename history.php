...
$username=$_SESSION['username'];  
$status=2;
$sql="SELECT SUM(Quantity) as Quantity FROM history WHERE Username=:username AND Status=:status";
$query = $dbh -> prepare($sql);
$query-> bindParam(':username', $username, PDO::PARAM_STR);
$query-> bindParam(':status', $status, PDO::PARAM_STR);
$query->execute();
...
foreach($results as $result)
{               ?>  
            <h4 class="header-line" >Checkout item history &nbsp<i class="fas fa-history"></i> &nbsp<?php echo htmlentities($result->Quantity);?></h4>
            <?php }} ?> 
...
$username=$_SESSION['username'];  
$status=2;
$sql="SELECT id,ProductCode,ProductName,ProductImage,ProductPrice,Quantity,TotalPrice,Username,ProductPrice*Quantity as Total,Status FROM history WHERE Username=:username AND Status=:status";
$query = $dbh -> prepare($sql);
$query-> bindParam(':username', $username, PDO::PARAM_STR);
$query-> bindParam(':status', $status, PDO::PARAM_STR);
$query->execute();
